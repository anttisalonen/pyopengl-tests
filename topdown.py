...
import time
import random
from math import radians, degrees, cos, sin, tan, pi, ceil
...
def drawWithVBOs(entities, state, lights, current_daytime):
    ...
    if perspective:
        ...
        r = state[controls.viewrange] / 2.0
        t = state[controls.viewrange] / 2.0
        f = 10.0
        n = 0.1
        pers[0][0] = 1.0 / r
        pers[1][1] = 1.0 / t
    ...
    if lights.ambientLightEnabled:
        ...
        glUniform3f(ambientLoc, 1.0, 1.0, 1.0)
    ...
    for entity in entities:
        if entity.radius and \
                abs(state[controls.pos][0] - entity.position[0]) > entity.radius + ceil(state[controls.viewrange]) / 2.0 and \
                abs(state[controls.pos][2] - entity.position[2]) > entity.radius + ceil(state[controls.viewrange]) / 2.0:
                continue
        # Texture
...
mp[K_UP] = lambda tgt, upv: pos_step * normalized(upv)
mp[K_DOWN] = lambda tgt, upv: -mp[K_UP](tgt, upv)
# mp[K_PAGEUP] = lambda tgt, upv: pos_step * normalized(tgt)
...
class TerrainModel(Model):
    def __init__(self, width, height):
        ...
        self.setup_indices(0, 0, 10, 10)

    def setup_indices(self, x1, y1, x2, y2):
        indices = []
        x1 = max(0, x1)
        y1 = max(0, y1)
    ...
    def heightmap(self, x, y):
        ...
        return 50.0 * pnoise2(x / self.freq, y / self.freq, self.octaves)
...
def main():
    ...
    mouse_look = False
    state = dict()
    state[controls.pos] = numpy.array([3, 8, 3], numpy.float32)
    state[controls.hrot] = 0
    state[controls.vrot] = radians(90)
    state[controls.viewrange] = 10
    ...
    while not done:
        xp = int(state[controls.pos][0])
        zp = int(state[controls.pos][2])
        model4.setup_indices(zp - int(state[controls.viewrange]), xp - int(state[controls.viewrange]),
                zp + int(state[controls.viewrange]), xp + int(state[controls.viewrange]))
        glViewport(0, 0, width, height)
        current_daytime = (pygame.time.get_ticks() / 120.0)
        if not lights.dayShift:
            current_daytime = 90
        ...
        for event in pygame.event.get():
            if event.type == KEYDOWN:
                if event.key in mp:
                    ...
                    lights.dayShift = not lights.dayShift
                elif event.key == K_PAGEUP:
                    if state[controls.viewrange] < 100:
                        state[controls.viewrange] += 2
                elif event.key == K_PAGEDOWN:
                    if state[controls.viewrange] > 2:
                        state[controls.viewrange] -= 2
